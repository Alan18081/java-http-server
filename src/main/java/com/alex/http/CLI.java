...
import com.alex.http.impl.HttpServerFactory;
...
public class CLI {
    ...
    private static final List<String> QUIT_COMMANDS = Collections.unmodifiableList(Arrays.asList(new String[] { "q", "quit", "exit" }));
    ...
    private static void waitForStopCommand(HttpServer httpServer) {
        try(Scanner scanner = new Scanner(System.in, StandardCharsets.UTF_8.name())) {
            while(true) {
                ...
                if(QUIT_COMMANDS.contains(cmd.toLowerCase())) {
                    httpServer.stop();
                    break;
                } else {
                    LOGGER.error("Unsupported command " + cmd + ". To shut down server, please type 'q'");
                }
            }
        }
    }
